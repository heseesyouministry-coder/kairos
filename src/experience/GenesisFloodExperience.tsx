...
import { floodScene } from "../content/genesis/flood/scene";
...
export const GenesisFloodExperience: React.FC = () => {
  ...
  useEffect(() => {
    ...
    AudioManager.playTrack("flood", "deluge-thunder");
    markSceneCompleted(floodScene.id);
  }, [setCurrentScene, markSceneCompleted]);

  const handleAdvance = () => {
    TransitionEngine.execute(
      floodScene.transition.type,
      floodScene.transition.to,
      {},
      navigate
    );
  };
  ...
  return (
    <div className="relative min-h-screen w-full bg-[#0c0d10] flex flex-col justify-center overflow-x-hidden">
      <AtmosphericCanvas environmentId={floodScene.environment.id} />

      <div className="relative z-10 flex flex-col items-center justify-center min-h-screen">
        <header className="pt-16 pb-4 text-center select-none">
          <div className="text-[11px] font-sans uppercase tracking-[0.3em] text-[#7aa3be]/80 font-medium">
            Genesis — The Flood
          </div>
          <h1 className="text-xl md:text-2xl font-serif text-[#e8e6df] tracking-wide mt-1">
            Ang Pagkawasak ng Tahimik na Daigdig
          </h1>
        </header>
