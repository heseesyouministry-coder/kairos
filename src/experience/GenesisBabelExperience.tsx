...
import { babelScene } from "../content/genesis/babel/scene";
...
export const GenesisBabelExperience: React.FC = () => {
  ...
  useEffect(() => {
    ...
    AudioManager.playTrack("babel", "tower-wind-voices");
    markSceneCompleted(babelScene.id);
  }, [setCurrentScene, markSceneCompleted]);

  const handleAdvance = () => {
    TransitionEngine.execute(
      babelScene.transition.type,
      babelScene.transition.to,
      {},
      navigate
    );
  };
  ...
  return (
    <div className="relative min-h-screen w-full bg-[#0c0d10] flex flex-col justify-center overflow-x-hidden">
      <AtmosphericCanvas environmentId={babelScene.environment.id} />
